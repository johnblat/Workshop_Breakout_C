...
// TASK:
//
// Let's start making levels. 
// Meaning that there are different layouts for bricks.
//
//
// Let's just hardcode a level layout.
//
// So, instead of just setting every brick to true, let's make it so that there is a section of bricks in the
// middle that are filled - which will basically be level 1 going forward.
// 
// in my code, the brick statuses are boolean. 
// So in code, you can use true/false, INACTIVE/ACTIVE defines i created, or the numbers 0/1 to refer to boolean values
